...
class Cursor_tools {
  ...
  constructor() {
    this.coordArray = [];
    this.abnormalAcceleration = [];
    this.abnormalAcceleration = false;
  }
  ...
  */
  calculateVelocity(coord1, coord2) {
    ...
    console.log("coord2  " + coord2.time_ms);
    console.log("Coord 1 " + coord1.time_ms);
    timeDifference = Math.abs(coord2.time_ms - coord1.time_ms);
 ...
 */
  calculateAcceleration(startVelocity, finalVelocity) {
  ...
  */
  historicalAccelerationComparison() {
    const maxAccelerationData = 100;
    var velocity1 = this.calculateVelocity(this.coordArray[0], this.coordArray[1]);
    var velocity2 = this.calculateVelocity(this.coordArray[2], this.coordArray[3]);
    var currAcceleration = this.calculateAcceleration(velocity1, velocity2);
    var maxAccelRatio = 1.5;
    var minAccelRatio = 0.5;
    ...
    if (this.accelerationsArray.length != maxAccelerationData) {
      ...
      if (accelerationRatio < minAccelRatio || accelerationRatio >= maxAccelRatio) {
        //console.log("ABNORMAL ACCEL!!!!!!");
        this.abnormalAcceleration = true;
      }
